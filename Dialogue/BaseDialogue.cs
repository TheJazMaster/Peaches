...
internal abstract class BaseDialogue(string charKey)
{
	internal readonly string charKey = charKey;
	...
	internal static string TranslateChar(string name) {
		return name switch {
			...
			"Jumbo" => "miner",
			"George" => "spike",
			"EvilRiggs" => "pirateBoss",
			"Bucket" => "TheJazMaster.Bucket::Bucket",
	...
	protected List<Instruction> ConvertToInstructions(List<object> list, string key) {
		...
		// Case: generic bucket line. First string is line, second is looptag
		if (list[0] is string str) {
			ret.Add(new Say {
				hash = "0",
				who = TranslateChar("Bucket"),
	...
	protected List<Say> ConvertToSays(List<object> list, string key, ref int hash) {
		...
		// Case: generic character line. First string is line, second is looptag
		if (list[0] is string str) {
			ret.Add(new Say {
				hash = hash.ToString(),
				who = TranslateChar(charKey),
				loopTag = list.Count > 1 ? (list[1] as string) : "neutral"
			});
			if (!dict.TryGetValue(key, out Dictionary<string, string>? value)) {
				dict.Add(key, new Dictionary<string, string> {
		...
		foreach (KeyValuePair<string, Dictionary<string, List<List<object>>>> kvp in dialogue) {
			foreach (KeyValuePair<string, List<List<object>>> pairs in kvp.Value) {
				for (int i = 0; i < pairs.Value.Count; i++) {
					...
					string key = kvp.Key.Replace("{{CharacterType}}", charKey);
					string advancedKey = prefix ? "TheJazMaster.UnseenEffort::" + key : key;
					...
					node.allPresent ??= [TranslateChar("Bucket")];
					if (pairs.Key != "Basic") {
